...
{
    // 打印  printf("k: %lu %f %f %f %f \n",k,Rho[ic],Vel[ic].x,Vel[ic].y,Vel[ic].z);
    // printf("888888888888888888888 rhoBC0 %f  and rhoBC1 %f\n",rhoBC[0] ,rhoBC[1]);
    ...
    // size_t NumofNneigh = lbmaux[0].Nneigh;

    auto calculateRhoAndVel = [&]()
    {
        Rho[ib] = 0.0;
    ...
    case BCT_ZMIN0:
        ...
        if (!IsSolid[ib])
        {
            ...
            calculateRhoAndVel();
            // dat.oss_ss <<  "rho" <<rho  << std::endl;
            // printf("zmin0   f[0]= %f , f[1] =%f, f[2] =%f, f[3] =%f, f[4] =%f, f[5] =%f, f[6] =%f, f[7] =%f, f[8] =%f, f[9] =%f, f[10] =%f, f[11] =%f, f[12] =%f, f[13] =%f, f[14] =%f\n", f[0], f[1], f[2], f[3], f[4], f[5], f[6], f[7], f[8], f[9], f[10], f[11], f[12], f[13], f[14]);
        }
...
void Setup(FLBM::Domain &dom, void *UD)
{
    ...
    //     // printf("rho0min = %f, rho1min = %f, rho0max = %f, rho1max = %f\n", rho0min, rho1min, rho0max, rho1max);
