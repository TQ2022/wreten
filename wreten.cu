...
#include <cmath>
#include <algorithm>
// Helper function to determine if a point lies within a sphere
bool within_sphere(std::vector<int> center, int x, int y, int z, int radius) {
...
void Setup(FLBM::Domain &dom, void *UD)
{
   ...
   // real rho = dat.Head * (std::sin(2*M_PI*dat.time/dat.Tf-M_PI)) + dat.Orig;
  // real rho = dat.Head * (2*dat.time/dat.Tf-1) + dat.Orig;
  real rho = dat.Head * (std::sin(2*M_PI*dat.time/dat.Tf-0.5*M_PI)) + dat.Orig;
    real rho0min;
    real rho1min;
    real rho0max;
    real rho1max;
...
void Report(FLBM::Domain &dom, void *UD)
{
    ...
    rho = dat.Head * (std::sin(2*M_PI*dat.time/dat.Tf-0.5*M_PI)) + dat.Orig;
...
/*
   int bound = outlimit;
    ...
    FLBM::Domain Dom(D3Q15, nua, iVec3_t(N, Ny, Nz), 1.0, 1.0);
       ...
       const int total_grid_size = 100;
    const int subgrid_size = 20;
    const int radius = 8;

    
    

    // Loop through sub-grids and create a sphere at the center of each sub-grid
    for (int grid_x = 0; grid_x <= total_grid_size; grid_x += subgrid_size) {
        for (int grid_y = 0; grid_y <= total_grid_size; grid_y += subgrid_size) {
            for (int grid_z = 0; grid_z <= total_grid_size; grid_z += subgrid_size) {
                // Determine the center of the sphere in the current sub-grid
                std::vector<int> center = {grid_x + subgrid_size / 2, grid_y + subgrid_size / 2, grid_z + subgrid_size / 2};

                // Fill in the points within the sphere inside this sub-grid
                for (int x = grid_x; x < std::min(grid_x + subgrid_size, total_grid_size); x++) {
                    for (int y = grid_y; y < std::min(grid_y + subgrid_size, total_grid_size); y++) {
                        for (int z = grid_z; z < std::min(grid_z + subgrid_size, total_grid_size); z++) {
                            if (within_sphere(center, x, y, z, radius)) {
